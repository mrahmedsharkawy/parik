...
import { BOT_ENGINE_VERSION, createTrainingEngine } from "../_shared/training-engine.generated.mjs";
...
const PARITY_DEBUG = String(Deno.env.get("BOT_PARITY_DEBUG") || "").toLowerCase() === "true";
...
async function verifiedAdmin(req:Request){const token=String(req.headers.get("authorization")||"").replace(/^Bearer\s+/i,"");if(!token||token===PUBLIC||token===SECRET)return false;const userRes=await fetch(`${SB}/auth/v1/user`,{headers:{apikey:PUBLIC,authorization:`Bearer ${token}`}});if(!userRes.ok)return false;const user=await userRes.json();if(!user?.id)return false;const q=new URLSearchParams({select:"id",user_id:`eq.${user.id}`,active:"eq.true",limit:"1"});const adminRes=await fetch(`${SB}/rest/v1/admins?${q}`,{headers:dbHeaders()});return adminRes.ok&&Boolean((await adminRes.json().catch(()=>[]))[0]);}
...
function unansweredKey(value:string){return String(value||"").toLowerCase().replace(/[إأآٱ]/g,"ا").replace(/ة/g,"ه").replace(/ى/g,"ي").replace(/[\u064B-\u065F\u0670ـ]/g,"").replace(/[^\u0600-\u06FFa-z0-9\s]/g," ").replace(/\s+/g," ").trim();}
