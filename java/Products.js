...
"function" != typeof window.setPageTitleI18n && (window.setPageTitleI18n = function(titleAr, titleEn) {
    try {
        const title = ("en" === (document.documentElement.lang || document.documentElement.getAttribute("lang") || "ar") ? titleEn || titleAr : titleAr || titleEn) || document.title;
...
function normalizeAssetUrl(u) {
    try {
        ...
        let fragment = "";
        -1 !== fragmentIndex && (fragment = (query || "").substring(fragmentIndex), query = (query || "").substring(0, fragmentIndex)), 
        path = path.trim().replace(/^\.\//, "").replace(/^\/+/, "").replace(/\/+$/, "");
        let result = path.split("/").map(seg => {
            try {
        ...
        return result = result.replace(/%2520/gi, "%20"), query && (result += "?" + query), 
...
export function createProductCard(prod) {
    ...
    let touchStartX = 0, touchStartY = 0, touchStartAt = 0, suppressProductOpenUntil = 0, productOpenStarted = false, touchStartedOnCard = false, touchMoved = false;
    if (typeof window !== "undefined") {
    ...
    card.addEventListener("touchmove", e => {
        ...
        if (Math.abs(dx) > 8 || Math.abs(dy) > 8) {
            touchMoved = true;
            suppressProductOpenUntil = Date.now() + 800;
        }
    }, { passive: true }),
    card.addEventListener("touchend", e => {
        ...
        const quickTap = Date.now() - touchStartAt <= 520 && Math.abs(dx) <= 8 && Math.abs(dy) <= 8 && !touchMoved;
        if (!quickTap) {
            suppressProductOpenUntil = Date.now() + 800;
    ...
    card.style.textAlign = "ltr" === productCardDir ? "left" : "right";
    function getTranslated(val) {
        return "object" == typeof val && null !== val ? val[lang] || val.ar || val.en || "" : val || "";
    }
    const savedCurrency = localStorage.getItem("currency") || ("en" === lang ? "USD" : "AED"), currencySymbol = (currency => {
        const symbols = {
            USD: {
                ar: "$",
                en: "$"
    ...
    content.setAttribute("dir", productCardDir);
    content.style.direction = productCardDir;
    content.style.textAlign = "ltr" === productCardDir ? "left" : "right";
    const name = document.createElement("div");
    if (name.className = "product-name", name.textContent = getTranslated(prod.name), 
    content.appendChild(name), prod.stock && parseInt(prod.stock) < 50) {
        const stock = document.createElement("div");
        stock.className = "product-stock", stock.textContent = "en" === lang ? `Stay ${prod.stock} only` : `يتبقي ${prod.stock} فقط `, 
        content.appendChild(stock);
    }
    if (prod.rating) {
        const ratingContainer = document.createElement("div");
        ratingContainer.className = "product-rating-container", ratingContainer.style.display = "flex", 
        ratingContainer.style.alignItems = "center", ratingContainer.style.gap = "4px";
        const starsContainer = document.createElement("div");
        starsContainer.className = "stars-container";
        const emptyStars = document.createElement("div");
        emptyStars.className = "stars-empty", emptyStars.innerHTML = "★★★★★";
        const filledStars = document.createElement("div");
        if (filledStars.className = "stars-filled", filledStars.style.width = `${Math.min(prod.rating / 5 * 100, 100)}%`, 
        filledStars.innerHTML = "★★★★★", "ar" === lang ? (filledStars.style.right = "0", 
        filledStars.style.left = "auto") : (filledStars.style.left = "0", filledStars.style.right = "auto"), 
        starsContainer.appendChild(emptyStars), starsContainer.appendChild(filledStars), 
        ratingContainer.appendChild(starsContainer), prod.ratingCount) {
            const ratingCount = document.createElement("span");
            ratingCount.className = "rating-count", ratingCount.textContent = prod.ratingCount, 
            ratingCount.style.fontSize = "0.95em", ratingCount.style.color = "#777", ratingCount.style.marginLeft = "4px", 
    ...
    prod.save) {
        ...
        save.className = "product-save", save.textContent = prod.save, filterRow.appendChild(save);
    }
    ...
    function handleCardAdd(ev) {
        ...
        const now = Date.now();
        if (now - lastAddTapTs < 220) return;
        lastAddTapTs = now;
        const rawImg = firstImage || prod.img || prod.image || "", safeImg = rawImg.startsWith("data:") ? "assets/logo.png" : rawImg, payload = {
            id: String(prod.id || prod.productId || ""),
            title: ("object" == typeof prod.name ? prod.name.ar || prod.name.en : prod.name || prod.title) || "",
            meta: prod.variant || prod.meta || "",
            img: safeImg,
            priceCurrent: "function" == typeof getNumericValue ? getNumericValue(prod.price || prod.priceCurrent || 0) : parseFloat(String(prod.price || 0).replace(/[^\d.-]/g, "")) || 0,
        ...
        if ("function" == typeof window.addProduct) try {
            ...
            "1" === localStorage.getItem("x2_logged") && window.Supabase && window.Supabase.UserSync && (clearTimeout(window._cartSyncTimer), 
            window._cartSyncTimer = setTimeout(() => window.Supabase.UserSync.push("cart"), 1500));
            const count = cart.reduce((s, it) => s + (Number(it.qty) || 1), 0), badge = document.getElementById("checkout-count");
            badge && (badge.textContent = count, badge.style.display = count > 0 ? "flex" : "none"), 
    ...
    cartBtn.addEventListener("touchstart", ev => {
        ...
        cartTouchStartX = ev.touches[0].clientX;
        cartTouchStartY = ev.touches[0].clientY;
        cartTouchStartAt = Date.now();
    }, { passive: true }), cartBtn.addEventListener("touchmove", ev => {
        if (!ev.touches || ev.touches.length !== 1) return;
        const dx = ev.touches[0].clientX - cartTouchStartX, dy = ev.touches[0].clientY - cartTouchStartY;
        if (Math.abs(dx) > 8 || Math.abs(dy) > 8) suppressCartAddUntil = Date.now() + 500;
    }, { passive: true }), cartBtn.addEventListener("touchend", ev => {
        if (Date.now() - cartTouchStartAt > 420 || Date.now() < suppressCartAddUntil) {
            ev.preventDefault();
            ev.stopPropagation();
...
document.addEventListener("DOMContentLoaded", async function() {
    ...
    function showSubCategories(categoryName, i18n, categoriesData) {
        ...
        if (categoryObj && categoryObj.subcategories && categoryObj.subcategories.length > 0) {
            if (categoryObj.subcategories.forEach(sub => {
                ...
                if (wrap.className = "subcategory-circle", sub.image) {
                    const img = document.createElement("img");
                    img.src = normalizeAssetUrl(sub.image), img.alt = sub.name.ar || sub.name.en || "", 
                    img.loading = "lazy", wrap.appendChild(img);
                }
                const subName = "rtl" === document.documentElement.dir ? sub.name.ar || sub.name.en : sub.name.en || sub.name.ar, label = document.createElement("span");
                ...
                const lang = localStorage.getItem("lang") || document.documentElement.lang || document.documentElement.getAttribute("lang") || "ar";
                subDisplay.innerHTML = "en" === lang ? "<p style='color:gray;'>No subcategories available.</p>" : "<p style='color:gray;'>لا توجد فئات فرعية.</p>";
            }
            return;
        }
        if (!sideMenu) return;
        const matched = Array.from(sideMenu.querySelectorAll(".category-item")).find(item => {
            const a = item.querySelector(":scope > a");
            if (!a) return !1;
            const s = a.querySelector("span[data-i18n]") || a.querySelector("span");
            return !!s && (!(!i18n || s.getAttribute("data-i18n") !== i18n) || s.textContent.trim() === categoryName);
        });
        if (!matched) return;
        const subLinks = matched.querySelectorAll(".flyout .subcategories a");
        0 !== subLinks.length && subLinks.forEach(sub => {
            const wrap = document.createElement("div");
            wrap.className = "subcategory-circle";
            const imgEl = sub.querySelector("img");
            if (imgEl) {
                const img = document.createElement("img");
                img.src = imgEl.src, img.alt = imgEl.alt || "", wrap.appendChild(img);
            }
            let subName = "";
            if (nameEl) {
        ...
        fetchProducts(forceFreshProducts).then(data => {
            ...
            if (titleEl) {
                ...
                document.title = titleEl.textContent; else {
                    ...
                    let catNameAr = categorySlug || "جميع الفئات", catNameEn = categorySlug || "All Categories";
                    if (categoryElement) {
                        const span = categoryElement.querySelector("span[data-i18n]");
                        span ? (catNameAr = span.textContent.trim(), catNameEn = span.getAttribute("data-i18n") || catNameAr) : (catNameAr = categoryElement.textContent.trim(), 
                        catNameEn = catNameAr);
                    }
                    const catName = "en" === lang ? catNameEn : catNameAr;
    ...
    window.addEventListener("popstate", function(event) {
        ...
        try {
            let lastScroll = JSON.parse(sessionStorage.getItem("categoryScrollPositions") || "{}")[categoryFromUrl];
            void 0 !== lastScroll ? setTimeout(() => {
                window.scrollTo({
    ...
    try {
        ...
        window.categoriesData = categoriesData, function(categoriesData) {
            if (!sideMenu) return;
            sideMenu.innerHTML = "";
            const allLabel = "rtl" === document.documentElement.dir ? "جميع الفئات" : "All Categories", allCategoriesDiv = document.createElement("div");
            allCategoriesDiv.className = "category-item all-categories-header", allCategoriesDiv.innerHTML = `\n      <a href="/categories.html">\n        💯 <span data-i18n="all-categories">${allLabel}</span>\n        <span class="arrow">›</span>\n      </a>\n    `, 
            sideMenu.appendChild(allCategoriesDiv), categoriesData.sort((a, b) => (a.order || 999) - (b.order || 999)).forEach(category => {
            ...
            allCategoriesHeader && allCategoriesHeader.addEventListener("click", function(e) {
                e.preventDefault(), titleEl && (titleEl.textContent = "جميع الفئات"), setPageTitleI18n("جميع الفئات", "All Categories"), 
                window.history.pushState({
                    category: "all"
                }, "", "categories.html?category=all"), loadProductsForCategory("all", window.categoriesData), 
                showSubCategories("جميع الفئات", "all-categories", window.categoriesData);
            });
        }(categoriesData), function() {
            if (!sideMenu) return;
            sideMenu.querySelectorAll(".category-item > a").forEach(link => {
                link.addEventListener("click", function(e) {
                    e.preventDefault(), window.scrollTo(0, 0);
                    const span = link.querySelector("span[data-i18n]"), catName = span ? span.textContent.trim() : link.textContent.trim(), i18n = span ? span.getAttribute("data-i18n") : "", slugMatch = (link.getAttribute("href") || "").match(/\/categories\/([^\/\?]+)/), categorySlug = slugMatch ? slugMatch[1] : catName;
                    try {
                        const scrollPositions = JSON.parse(sessionStorage.getItem("scrollPositions") || "{}");
                        scrollPositions[window.location.href] = window.scrollY, sessionStorage.setItem("scrollPositions", JSON.stringify(scrollPositions));
                    } catch (e) {}
                    window.scrollTo({
                        top: 0,
                        behavior: "auto"
                    ...
                    window.history.pushState({
                        ...
                        scrollToTop: !0
                    }, "", url), loadProductsForCategory(categorySlug, window.categoriesData), showSubCategories(catName, i18n, window.categoriesData);
                });
            });
            const allCategoriesHeader = sideMenu.querySelector(".all-categories-header > a");
            allCategoriesHeader && allCategoriesHeader.addEventListener("click", function(e) {
                e.preventDefault();
                const allLabel = "en" === (document.documentElement.lang || document.documentElement.getAttribute("lang") || "ar") ? "All Categories" : "جميع الفئات";
                titleEl && (titleEl.textContent = allLabel), document.title = allLabel, window.history.pushState({
                    category: "all"
                }, "", "categories.html?category=all"), loadProductsForCategory("all", window.categoriesData), 
                showSubCategories(allLabel, "all-categories", window.categoriesData);
            }), sideMenu.querySelectorAll(".subcategories a"), sideMenu.querySelectorAll(".subcategories").forEach(ul => {
                ul.addEventListener("click", function(e) {
                    const link = e.target.closest("a");
                    if (!link) return;
                    e.preventDefault(), window.scrollTo(0, 0);
                    const href = link.getAttribute("href") || "", urlParams = new URLSearchParams(href.split("?")[1] || ""), parentSlug = urlParams.get("category"), subSlug = urlParams.get("subcategory"), span = link.querySelector("span"), subName = span ? span.textContent.trim() : link.textContent.trim(), url = `categories.html?category=${encodeURIComponent(parentSlug || "")}&subcategory=${encodeURIComponent(subSlug || "")}`;
                    window.history.pushState({
                        category: parentSlug,
                        subcategory: subSlug,
                        forceScrollTop: !0,
                    ...
                    const norm = v => "string" == typeof v ? v.toLowerCase().trim() : v, subNameNorm = norm(subName), subSlugNorm = norm(subSlug);
                    let subNameAr = subName, subNameEn = subName;
                    span && span.hasAttribute("data-i18n") && (subNameAr = span.getAttribute("data-i18n"), 
                    subNameEn = span.textContent.trim(), "en" === document.documentElement.lang && ([subNameAr, subNameEn] = [ subNameEn, subNameAr ]));
                    const subNameArNorm = norm(subNameAr), subNameEnNorm = norm(subNameEn);
                    function matchValue(val) {
                        return !!val && ("object" != typeof val || null === val || Array.isArray(val) ? Array.isArray(val) ? val.some(item => matchValue(item)) : norm(val) === subNameNorm || norm(val) === subSlugNorm || norm(val) === subNameArNorm || norm(val) === subNameEnNorm : val.ar && (norm(val.ar) === subNameNorm || norm(val.ar) === subSlugNorm || norm(val.ar) === subNameArNorm || norm(val.ar) === subNameEnNorm) || val.en && (norm(val.en) === subNameNorm || norm(val.en) === subSlugNorm || norm(val.en) === subNameArNorm || norm(val.en) === subNameEnNorm));
                    }
                    const filtered = products.filter(p => matchValue(p.subCategory) || matchValue(p.subcategory) || matchValue(p.category) || matchValue(p.mainCategory) || p.subcategorySlug && norm(p.subcategorySlug) === subSlugNorm || p.subCategorySlug && norm(p.subCategorySlug) === subSlugNorm);
                    ...
                    titleEl && (titleEl.textContent = subName, document.title = subName), setTimeout(() => {
        ...
        try {
            const currentLang = localStorage.getItem("lang") || document.documentElement.lang || ("rtl" === document.documentElement.dir ? "ar" : "en");
            "function" == typeof window.applyCategoryNames && window.applyCategoryNames(currentLang);
        } catch (e) {}
        categoriesContainer && categoriesContainer.querySelectorAll("div").forEach(function(div, index) {
            ...
            link && (link.addEventListener("click", function(e) {
                ...
                const currentCategoryId = new URLSearchParams(window.location.search).get("category") || "all", currentIndex = Array.from(categoriesContainer.querySelectorAll("div")).findIndex(d => {
                    ...
                    if (!a) return !1;
                    const slugMatch = (a.getAttribute("href") || "").match(/\/categories\/([^\/\?]+)/);
    ...
    if (filterDropdowns.forEach(function(filterDropdown) {
        const filterToggle = filterDropdown.querySelector(".filter-toggle");
        filterToggle && filterToggle.addEventListener("click", function(e) {
            e.stopPropagation(), filterDropdowns.forEach(d => {
                d !== filterDropdown && d.classList.remove("open");
        ...
        filterDropdowns.forEach(function(filterDropdown) {
            if (!filterDropdown.contains(e.target)) {
                ...
                options && (options.style.display = "");
            }
        });
    }), categoriesContainer) {
        let startXCategories, scrollLeftCategories, isDownCategories = !1;
        categoriesContainer.addEventListener("mousedown", e => {
            isDownCategories = !0, categoriesContainer.classList.add("active"), startXCategories = e.pageX - categoriesContainer.offsetLeft, 
            scrollLeftCategories = categoriesContainer.scrollLeft, e.preventDefault(), categoriesContainer.querySelectorAll("a").forEach(a => a.style.pointerEvents = "none");
        }), categoriesContainer.addEventListener("mouseleave", () => {
            isDownCategories = !1, categoriesContainer.classList.remove("active"), categoriesContainer.querySelectorAll("a").forEach(a => a.style.pointerEvents = "");
        }), categoriesContainer.addEventListener("mouseup", () => {
            isDownCategories = !1, categoriesContainer.classList.remove("active"), categoriesContainer.querySelectorAll("a").forEach(a => a.style.pointerEvents = "");
        }), categoriesContainer.addEventListener("mousemove", e => {
            if (!isDownCategories) return;
            e.preventDefault();
            const walk = e.pageX - categoriesContainer.offsetLeft - startXCategories;
            categoriesContainer.scrollLeft = scrollLeftCategories - walk;
        });
    }
    if (filtersScroll) {
        let startXFilters, scrollLeftFilters, isDownFilters = !1;
        filtersScroll.addEventListener("mousedown", e => {
            isDownFilters = !0, filtersScroll.classList.add("active"), startXFilters = e.pageX - filtersScroll.offsetLeft, 
            scrollLeftFilters = filtersScroll.scrollLeft;
        }), filtersScroll.addEventListener("mouseleave", () => {
            isDownFilters = !1, filtersScroll.classList.remove("active");
        }), filtersScroll.addEventListener("mouseup", () => {
            isDownFilters = !1, filtersScroll.classList.remove("active");
        }), filtersScroll.addEventListener("mousemove", e => {
            if (!isDownFilters) return;
            e.preventDefault();
            const walk = e.pageX - filtersScroll.offsetLeft - startXFilters;
            filtersScroll.scrollLeft = scrollLeftFilters - walk;
        });
    }
    filtersScroll && filtersScroll.querySelectorAll('input[type="checkbox"], input[type="radio"]').forEach(input => {
    ...
    function applyFilters() {
        filtersScroll.querySelectorAll(".filter-dropdown").forEach(dropdown => {
            ...
            if (filterToggle && !filterToggle.hasAttribute("data-filter-id")) {
                const filterText = filterToggle.textContent.trim();
                let filterId = "";
                filterId = filterText.includes("فلتر بالسعر") || filterText.includes("Price") ? "price" : filterText.includes("الأحدث") || filterText.includes("Newest") ? "newest" : filterText.includes("التقييم") || filterText.includes("Rating") ? "rating" : filterText.includes("اللون") || filterText.includes("Color") ? "color" : filterText.includes("الحجم") || filterText.includes("Size") ? "size" : filterText.includes("العلامة التجارية") || filterText.includes("Brand") ? "brand" : filterText.includes("التصنيف") || filterText.includes("Category") ? "category" : filterText.includes("المخزون") || filterText.includes("Stock") ? "stock" : filterText.includes("المراجعات") || filterText.includes("Reviews") ? "reviews" : filterText.includes("تاريخ الإضافة") || filterText.includes("Date") ? "date" : filterText.replace(/[\u{1F300}-\u{1F6FF}]/gu, "").trim().toLowerCase().replace(/\s+/g, "-"), 
        ...
        filtersScroll.querySelectorAll(".filter-dropdown").forEach(dropdown => {
            ...
            checkedInputs.length > 0 && (filters[filterId] = Array.from(checkedInputs).map(input => input.value));
        });
        let filtered = products.slice();
        filters.price && filters.price.length > 0 && (filtered = function(products, priceFilters) {
        ...
        }(filtered, filters.price)), filters.newest && filters.newest.length > 0 && (filtered = function(products, sortType) {
            ...
            switch (sortType) {
              case "latest":
                return sorted.sort((a, b) => {
                    ...
                    return isNaN(dateA) || isNaN(dateB) ? (b.id || 0) - (a.id || 0) : dateB - dateA;
                });
              ...
              case "oldest":
                return sorted.sort((a, b) => {
                    ...
                    if (!b.date) return -1;
                    const dateA = new Date(a.date).getTime(), dateB = new Date(b.date).getTime();
                    return isNaN(dateA) || isNaN(dateB) ? (a.id || 0) - (b.id || 0) : dateA - dateB;
                });
        ...
        }(filtered, filters.newest[0])), filters.rating && filters.rating.length > 0 && (filtered = function(products, ratingFilters) {
            return products.filter(prod => {
        ...
        }(filtered, filters.rating)), filters.color && filters.color.length > 0 && (filtered = filterByAttribute(filtered, "color", filters.color)), 
        filters.size && filters.size.length > 0 && (filtered = filterByAttribute(filtered, "size", filters.size)), 
        filters.brand && filters.brand.length > 0 && (filtered = filterByAttribute(filtered, "brand", filters.brand)), 
        filters.category && filters.category.length > 0 && (filtered = function(products, categoryFilters) {
            return products.filter(prod => {
                const normalizedCategoryFilters = categoryFilters.map(c => String(c).toLowerCase().trim()), categoryFields = [ prod.category && "string" == typeof prod.category ? prod.category.toLowerCase().trim() : "", prod.mainCategory && "string" == typeof prod.mainCategory ? prod.mainCategory.toLowerCase().trim() : "", prod.categoryName && "string" == typeof prod.categoryName ? prod.categoryName.toLowerCase().trim() : "", prod.categorySlug ? prod.categorySlug.toLowerCase().trim() : "", prod.mainCategorySlug ? prod.mainCategorySlug.toLowerCase().trim() : "" ].filter(Boolean);
                return prod.category && "object" == typeof prod.category && null !== prod.category && (prod.category.ar && categoryFields.push(prod.category.ar.toLowerCase().trim()), 
                prod.category.en && categoryFields.push(prod.category.en.toLowerCase().trim())), 
                prod.mainCategory && "object" == typeof prod.mainCategory && null !== prod.mainCategory && (prod.mainCategory.ar && categoryFields.push(prod.mainCategory.ar.toLowerCase().trim()), 
                prod.mainCategory.en && categoryFields.push(prod.mainCategory.en.toLowerCase().trim())), 
                categoryFields.some(field => normalizedCategoryFilters.some(filterCat => field.includes(filterCat) || filterCat.includes(field)));
            });
        }(filtered, filters.category)), filters.stock && filters.stock.length > 0 && (filtered = function(products, stockFilters) {
            return products.filter(prod => {
                const stock = getNumericValue(prod.stock);
                return stockFilters.includes("in") ? stock > 0 : !stockFilters.includes("out") || 0 === stock;
            });
        }(filtered, filters.stock)), filters.reviews && filters.reviews.length > 0 && (filtered = function(products, reviewFilters) {
            return products.filter(prod => {
                const ratingCount = getNumericValue(prod.ratingCount || prod.reviewCount || 0);
                return reviewFilters.includes("with") ? ratingCount > 0 : !reviewFilters.includes("none") || 0 === ratingCount;
            });
        }(filtered, filters.reviews)), filters.date && filters.date.length > 0 && (filtered = function(products, dateFilters) {
            const now = Date.now();
            return products.filter(prod => {
                if (!prod.date) return !1;
                try {
                    let prodDate;
                    if ("string" == typeof prod.date) prodDate = new Date(prod.date).getTime(); else {
                        if ("number" != typeof prod.date) return !1;
                        prodDate = prod.date;
                    }
                    return !isNaN(prodDate) && dateFilters.some(filter => "24h" === filter ? now - prodDate <= 864e5 : "week" === filter ? now - prodDate <= 6048e5 : "month" === filter ? now - prodDate <= 2592e6 : "year" !== filter || now - prodDate <= 31536e6);
                } catch (e) {
                    return console.error("خطأ في معالجة التاريخ:", e, prod.date), !1;
                }
            });
    ...
    function filterByAttribute(products, attribute, filterValues) {
        return products.filter(prod => {
            if (!prod[attribute]) return !1;
            const attributeValue = prod[attribute], normalizedFilterValues = filterValues.map(v => String(v).toLowerCase().trim());
            if (Array.isArray(attributeValue)) {
                return attributeValue.map(v => String(v).toLowerCase().trim()).some(val => normalizedFilterValues.some(filterVal => val.includes(filterVal) || filterVal.includes(val)));
            }
            if ("object" == typeof attributeValue && null !== attributeValue) {
                const lang = document.documentElement.lang || document.documentElement.getAttribute("lang") || "ar", localizedValue = String(attributeValue[lang] || attributeValue.ar || attributeValue.en || "").toLowerCase().trim();
                return normalizedFilterValues.some(filterVal => localizedValue.includes(filterVal) || filterVal.includes(localizedValue));
            }
            const normalizedValue = String(attributeValue).toLowerCase().trim();
            return normalizedFilterValues.some(filterVal => normalizedValue.includes(filterVal) || filterVal.includes(normalizedValue));
        });
    }
    function getNumericValue(value) {
        ...
        if ("object" == typeof value && null !== value) {
            const lang = document.documentElement.lang || document.documentElement.getAttribute("lang") || "ar", cleanValue = String(value[lang] || value.ar || value.en || "").replace(/[^\d.-]/g, "");
            return parseFloat(cleanValue) || 0;
        }
        return 0;
    }
    resetFiltersBtn && resetFiltersBtn.addEventListener("click", function() {
        filtersScroll.querySelectorAll('input[type="checkbox"]:checked, input[type="radio"]:checked').forEach(input => {
            input.checked = !1;
        }), applyFilters();
    }), filtersScroll && filtersScroll.querySelectorAll(".filter-dropdown").forEach(dropdown => {
        if (dropdown.querySelector(".clear-filter-btn")) return;
        const filterOptions = dropdown.querySelector(".filter-options");
        if (!filterOptions) return;
        const clearButton = document.createElement("button");
        clearButton.className = "clear-filter-btn", clearButton.textContent = "rtl" === document.documentElement.dir ? "مسح الفلتر" : "Clear filter", 
        clearButton.addEventListener("click", function(e) {
            e.stopPropagation(), dropdown.querySelectorAll('input[type="checkbox"]:checked, input[type="radio"]:checked').forEach(input => {
                input.checked = !1;
            }), applyFilters(), dropdown.classList.remove("open");
    ...
    fetchProducts(!0).then(all => {
        ...
        if (!p) {
            ...
            return void (pp && (pp.innerHTML = '<p style="padding:40px;color:#888;">المنتج غير موجود</p>'));
        }
        const sym = function(currency) {
            const s = {
                USD: {
                    ar: "$",
                    en: "$"
            ...
            return s[currency] && (s[currency][lang] || s[currency].en) || ("en" === lang ? "$" : "د.إ");
        ...
        thumbs && (thumbs.innerHTML = "", media.forEach((item, i) => {
            let thumb;
            if ("video" === item.type) {
                thumb = document.createElement("div"), thumb.className = "thumb-video";
                const v = document.createElement("video");
                v.src = item.src, v.muted = !0, v.preload = "metadata", v.playsInline = !0, v.style.width = "100%", 
                v.style.height = "100%", v.style.objectFit = "cover";
                const play = document.createElement("span");
                play.textContent = "▶", play.style.cssText = "position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);color:#fff;font-size:15px;text-shadow:0 1px 3px rgba(0,0,0,.6);pointer-events:none;", 
                thumb.appendChild(v), thumb.appendChild(play);
        ...
        r = parseFloat(r) || 0, "★".repeat(Math.floor(r)) + (r % 1 >= .5 ? "½" : "") + "☆".repeat(5 - Math.ceil(r))}</span> <b>${p.rating || ""}</b>`), 
        set("ratingCount", p.ratingCount ? `(${p.ratingCount} ${"en" === lang ? "reviews" : "تقييم"})` : "");
        const compactSales = function(productId) {
            const baseRange = [ 1.5, 2.3, 3.7, 4.2, 5.6, 6.8, 2.8, 3.2, 4.9, 5.2 ], baseValue = baseRange[("string" == typeof productId ? productId.split("").reduce((a, b) => a + b.charCodeAt(0), 0) : productId || 0) % baseRange.length], day = (new Date).getDate(), variation = day % 10 / 10 * .3;
            return (day % 2 == 0 ? baseValue + variation : baseValue - variation).toFixed(1) + "k+";
        }(p.id || p.productId);
        set("sales", "en" === lang ? `sold ${compactSales}` : `تم بيع ${compactSales}`);
        const hasDisc = p.oldPrice && parseFloat(p.oldPrice) > parseFloat(p.price);
        set("price", null != p.price ? `${p.price} ${sym}` : "");
        const oldEl = document.getElementById("oldPrice");
        oldEl && (oldEl.textContent = hasDisc ? `${p.oldPrice} ${sym}` : "", oldEl.style.display = hasDisc ? "" : "none");
        const discEl = document.getElementById("discount");
        if (discEl) if (hasDisc) {
        ...
        document.getElementById("minus")?.addEventListener("click", () => {
            ...
            qty && (qty.value = parseInt(qty.value || 1) + 1);
        });
        function addToCart() {
            const item = {
                id: p.id,
                title: getT(p.name),
                img: imgs[0] || "",
                priceCurrent: parseFloat(p.price) || 0,
                priceOld: parseFloat(p.oldPrice) || 0,
                qty: parseInt(qty?.value) || 1
            };
            if ("function" == typeof window.addProduct) window.addProduct(item); else try {
        ...
        const minusBtn = document.getElementById("minus"), plusBtn = document.getElementById("plus"), buyBtn = document.querySelector(".product-page button.buy");
        minusBtn && (minusBtn.type = "button"), plusBtn && (plusBtn.type = "button"), buyBtn && (buyBtn.type = "button"), 
        cartBtn && (cartBtn.type = "button");
        let productAddTapTs = 0;
        const onProductAdd = ev => {
            ev && (ev.preventDefault(), ev.stopPropagation());
            const now = Date.now();
            if (now - productAddTapTs < 220) return;
            productAddTapTs = now;
            addToCart();
            const sp = cartBtn.querySelector("span") || cartBtn, orig = sp.textContent;
            sp.textContent = "en" === lang ? "✓ Added!" : "✓ أضيف!", setTimeout(() => {
                sp.textContent = orig;
            }, 2e3);
        };
        cartBtn && (cartBtn.addEventListener("click", onProductAdd), cartBtn.addEventListener("touchend", onProductAdd, {
            passive: !1
        }));
        const waBtn = document.getElementById("whatsappOrderBtn");
        waBtn && (waBtn.removeAttribute("href"), waBtn.style.cursor = "pointer", waBtn.addEventListener("click", async function(e) {
            ...
            const qtyVal = Math.max(1, Number(document.getElementById("qty")?.value || 1)), total = ((parseFloat(p.price) || 0) * qtyVal).toFixed(2), readJson = (key, fb) => {
                try {
            ...
            const rawImg = Array.isArray(p.img) ? p.img[0] : p.img || p.image || "", newOrder = {
                id: orderId,
            ...
            try {
                ...
                const af = prof.address_full || {};
                customerName = customerName || prof.name || "";
                ...
                if (/(?:google\.[^/]+\/maps|maps\.google|goo\.gl\/maps|^https?:\/\/|@?-?\d{1,3}\.\d+\s*,\s*-?\d{1,3}\.\d+)/i.test(customerAddress)) {
                    customerAddress = [ af.city, af.area, af.state ].filter(Boolean).join("، ") || "";
                    if (prof.address && /(?:google\.[^/]+\/maps|maps\.google|goo\.gl\/maps|^https?:\/\/|@?-?\d{1,3}\.\d+\s*,\s*-?\d{1,3}\.\d+)/i.test(prof.address)) {
                        prof.address = customerAddress;
                        localStorage.setItem("x2_profile", JSON.stringify(prof));
                    }
                }
            } catch (e) {}
            if (!customerAddress) customerAddress = x2VisitorAreaFallback();
            if (!customerAddress && navigator.geolocation) {
                try {
                    customerAddress = await new Promise(resolve => {
                        let done = false;
                        const finish = v => {
                            if (done) return;
                            done = true;
            ...
            window.open(`https://wa.me/971544046084?text=${msg}`, "_self");
        ...
        if (relEl && "function" == typeof createProductCard) {
            ...
            function wordScore(a, b) {
                const wa = a.split(/[\s,،_\-\/]+/).filter(w => w.length > 1);
                const wb = b.split(/[\s,،_\-\/]+/).filter(w => w.length > 1);
                if (!wa.length || !wb.length) return 0;
                let m = 0;
                wa.forEach(w => {
                    if (wb.some(w2 => w2.includes(w) || w.includes(w2))) m++;
                });
                return m / Math.max(wa.length, wb.length);
            }
            const pCats = getPCats(p), pName = getT(p.name || "").toLowerCase();
            const rel = all.filter(x => String(x.id) !== String(productId)).map(x => {
                const xCats = getPCats(x), xName = getT(x.name || "").toLowerCase();
                const catScore = pCats.some(c => xCats.some(xc => xc === c || xc.includes(c) || c.includes(xc))) ? .8 : 0;
                const nameScore = wordScore(pName, xName) * 1.2;
                return {
                    x: x,
                    score: catScore + nameScore
                };
        ...
        (function() {
            ...
            const startMiniVideo = function() {
                ...
                pip.addEventListener("click", function(e) {
                    if (e.target === closeBtn || closeBtn.contains(e.target) || moved) return;
                    e.stopPropagation();
                    e.preventDefault();
                    try {
                        ...
                        fv.autoplay = true;
                        fv.playsInline = true;
                        fv.style.cssText = "position:fixed;inset:0;width:100%;height:100%;background:#000;z-index:100000;object-fit:contain;";
                        const overlay = document.createElement("div");
                        overlay.style.cssText = "position:fixed;inset:0;background:#000;z-index:99999;";
                        const cx = document.createElement("button");
                        cx.innerHTML = "&times;";
                        cx.style.cssText = "position:fixed;top:16px;right:16px;z-index:100001;background:rgba(0,0,0,.7);color:#fff;border:none;border-radius:50%;width:38px;height:38px;font-size:24px;cursor:pointer;display:flex;align-items:center;justify-content:center;";
                        function cleanup() {
                            try {
                                fv.pause();
                            } catch (e) {}
                ...
                pip.addEventListener("touchstart", onStart, {
                    passive: false
                });
