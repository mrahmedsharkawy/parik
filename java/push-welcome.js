(function(){
  ...
  async function activatePushFromWelcome(btn){
    ...
    try{
      ...
      if(!reg)reg=await navigator.serviceWorker.register('/sw.js?v=333',{updateViaCache:'none'});
  ...
  document.addEventListener('click',function(e){
    var later=e.target&&e.target.closest&&e.target.closest('#push-welcome-later-btn');
    if(later){
      e.preventDefault();
      e.stopPropagation();
      window.dismissPushWelcome();
      return;
    }
