(function () {
  ...
  var REFRESH_KEY = 'sw-v333-encoding-cleanup';
  var refreshed = false;
  ...
  function clearOldCaches() {
    ...
    caches.keys().then(function (keys) {
      keys.forEach(function (key) {
        if (key !== 'bariq-v333') caches.delete(key).catch(function () {});
      });
    }).catch(function () {});
  }
  ...
  navigator.serviceWorker.register('/sw.js?v=333', { updateViaCache: 'none' }).then(function (reg) {
    clearOldCaches();
