(function () {
  ...
  var REFRESH_KEY = 'sw-v291-pwa-scroll-refresh';
  var refreshed = false;
  ...
  function clearOldCaches() {
    ...
    caches.keys().then(function (keys) {
      keys.forEach(function (key) {
        if (key !== 'bariq-v291') caches.delete(key).catch(function () {});
      });
    }).catch(function () {});
  }
  ...
  navigator.serviceWorker.register('/sw.js?v=291', { updateViaCache: 'none' }).then(function (reg) {
    clearOldCaches();
