      ...
      Expanded(
        child: RefreshIndicator(
          ...
          child: NotificationListener<ScrollNotification>(
            ...
            child: ListView(
              padding: EdgeInsets.fromLTRB(10, 12, 10, _tab == 4 ? 130 : 72),
              children: [
...
class _PartnerStatus extends StatelessWidget { const _PartnerStatus({required this.partner,required this.english,required this.onEdit}); final AffiliatePartner partner; final bool english; final VoidCallback onEdit; @override Widget build(BuildContext context){final terminated=partner.status=='terminated';final labels={'pending':english?'Under review':'قيد المراجعة','suspended':english?'Suspended':'موقوف','rejected':english?'Rejected':'مرفوض','terminated':english?'Contract ended':'تم إنهاء التعاقد'};return Center(child:Padding(padding:const EdgeInsets.all(24),child:Container(padding:const EdgeInsets.all(24),decoration:BoxDecoration(color:Colors.white,borderRadius:BorderRadius.circular(20),border:Border.all(color:AppTheme.line)),child:Column(mainAxisSize:MainAxisSize.min,children:[Icon(partner.status=='pending'?Icons.hourglass_top_rounded:terminated?Icons.person_off_rounded:Icons.info_outline_rounded,color:terminated?Colors.redAccent:AppTheme.gold,size:52),const SizedBox(height:12),Text(labels[partner.status]??partner.status,style:const TextStyle(color:AppTheme.navy,fontSize:20,fontWeight:FontWeight.w900)),const SizedBox(height:8),Text(terminated?(english?'Your affiliate tools and referral links are no longer active. Your previous financial records are preserved.':'توقفت أدوات التسويق وروابط الإحالة، مع الاحتفاظ بسجلات المبيعات والعمولات السابقة.'):(english?'Marketing and withdrawal tools become available after admin approval.':'تتوفر أدوات التسويق والسحب بعد موافقة الإدارة.'),textAlign:TextAlign.center,style:const TextStyle(color:AppTheme.muted,height:1.5)),if(!terminated)...[const SizedBox(height:15),OutlinedButton(onPressed:onEdit,child:Text(english?'Review application':'مراجعة بيانات الطلب'))]]))));}}
...
class _AffiliateProfile extends StatefulWidget {
  const _AffiliateProfile({required this.partner,required this.service,required this.english,required this.onChanged});
  final AffiliatePartner partner;
  final AffiliateService service;
  final bool english;
  final Future<void> Function() onChanged;
  @override State<_AffiliateProfile> createState()=>_AffiliateProfileState();
}

class _AffiliateProfileState extends State<_AffiliateProfile> {
  late final TextEditingController _name,_account,_phone,_emirate,_instagram,_tiktok,_other,_method,_payoutMethod,_payoutDetails;
  bool _saving=false;
  @override void initState(){super.initState();_name=TextEditingController(text:widget.partner.fullName);_account=TextEditingController(text:widget.partner.accountName);_phone=TextEditingController(text:widget.partner.phone);_emirate=TextEditingController(text:widget.partner.emirate);_instagram=TextEditingController(text:widget.partner.instagram);_tiktok=TextEditingController(text:widget.partner.tiktok);_other=TextEditingController(text:widget.partner.otherSocial);_method=TextEditingController(text:widget.partner.marketingMethod);_payoutMethod=TextEditingController(text:widget.partner.payoutMethod);_payoutDetails=TextEditingController(text:'${widget.partner.payoutDetails['details']??''}');}
  @override void dispose(){for(final c in [_name,_account,_phone,_emirate,_instagram,_tiktok,_other,_method,_payoutMethod,_payoutDetails]){c.dispose();}super.dispose();}

  Future<void> _save()async{
    if(_name.text.trim().isEmpty||_account.text.trim().isEmpty||_phone.text.trim().isEmpty)return;
    setState(()=>_saving=true);
    try{
      await widget.service.updateProfile({'full_name':_name.text,'account_name':_account.text,'phone':_phone.text,'emirate':_emirate.text,'instagram':_instagram.text,'tiktok':_tiktok.text,'other_social':_other.text,'marketing_method':_method.text,'payout_method':_payoutMethod.text,'payout_details':{'details':_payoutDetails.text}});
      if(mounted)ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text(widget.english?'Profile saved':'تم حفظ الملف الشخصي')));
  ...
  Future<void> _terminate()async{
    ...
    final accepted=await showDialog<bool>(context:context,builder:(context)=>AlertDialog(
      title:Text(widget.english?'End affiliate contract':'إنهاء التعاقد'),
      content:Column(mainAxisSize:MainAxisSize.min,children:[Text(widget.english?'Your links and marketing tools will stop immediately. Previous commissions and records will remain preserved.':'ستتوقف روابطك وأدوات التسويق فورًا، مع الاحتفاظ بسجلاتك وعمولاتك السابقة.'),const SizedBox(height:12),TextField(controller:reason,maxLines:3,decoration:InputDecoration(labelText:widget.english?'Reason (optional)':'السبب (اختياري)'))]),
      actions:[TextButton(onPressed:()=>Navigator.pop(context,false),child:Text(widget.english?'Cancel':'إلغاء')),FilledButton(style:FilledButton.styleFrom(backgroundColor:Colors.redAccent),onPressed:()=>Navigator.pop(context,true),child:Text(widget.english?'End contract':'إنهاء التعاقد'))],
    ));
    final value=reason.text;reason.dispose();
    if(accepted!=true)return;
    try{await widget.service.terminateContract(reason:value);await widget.onChanged();}
    catch(e){if(mounted)ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text('$e')));}
  }

  @override Widget build(BuildContext context){
    final link=widget.service.partnerLink(widget.partner.code);
    return Column(crossAxisAlignment:CrossAxisAlignment.stretch,children:[
      Text(widget.english?'My affiliate profile':'ملفي كشريك',textAlign:TextAlign.start,style:const TextStyle(color:AppTheme.navy,fontSize:18,fontWeight:FontWeight.w900)),const SizedBox(height:10),
      Container(padding:const EdgeInsets.all(14),decoration:BoxDecoration(color:AppTheme.navy,borderRadius:BorderRadius.circular(16)),child:Column(crossAxisAlignment:CrossAxisAlignment.stretch,children:[Text(widget.english?'My marketing system':'نظام التسويق الخاص بي',style:const TextStyle(color:Colors.white,fontWeight:FontWeight.w900,fontSize:16)),const SizedBox(height:7),SelectableText(link,style:const TextStyle(color:Colors.white70,fontSize:11)),const SizedBox(height:9),Wrap(spacing:8,runSpacing:8,children:[FilledButton.icon(onPressed:(){Clipboard.setData(ClipboardData(text:link));ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text(widget.english?'Link copied':'تم نسخ رابطك')));},icon:const Icon(Icons.copy_rounded,size:17),label:Text(widget.english?'Copy my link':'نسخ رابط التسويق')),OutlinedButton.icon(style:OutlinedButton.styleFrom(foregroundColor:Colors.white,side:const BorderSide(color:Colors.white54)),onPressed:()=>Share.share('${widget.english?'Shop Bariq Gifts':'تسوق هدايا بريق'}\n$link'),icon:const Icon(Icons.share_rounded,size:17),label:Text(widget.english?'Share':'مشاركة'))])])),
      const SizedBox(height:12),
      _profileField(_name,widget.english?'Full name':'الاسم الكامل',required:true),_profileField(_account,widget.english?'Account name':'اسم حساب الشريك',required:true),_profileField(_phone,widget.english?'Phone':'رقم الهاتف',required:true),_profileField(_emirate,widget.english?'Emirate':'الإمارة'),
      Text(widget.english?'Marketing accounts':'حسابات التسويق',style:const TextStyle(color:AppTheme.navy,fontWeight:FontWeight.w900)),const SizedBox(height:7),
      _profileField(_instagram,'Instagram'),_profileField(_tiktok,'TikTok'),_profileField(_other,widget.english?'Other social account':'حساب تواصل آخر'),_profileField(_method,widget.english?'My marketing plan':'طريقة التسويق',maxLines:2),
      Text(widget.english?'Payout account':'حساب استلام الأرباح',style:const TextStyle(color:AppTheme.navy,fontWeight:FontWeight.w900)),const SizedBox(height:7),
      _profileField(_payoutMethod,widget.english?'Payout method':'طريقة الاستلام'),_profileField(_payoutDetails,widget.english?'Account / IBAN details':'بيانات الحساب / IBAN',maxLines:2),
      FilledButton.icon(onPressed:_saving?null:_save,style:FilledButton.styleFrom(backgroundColor:AppTheme.navy,minimumSize:const Size.fromHeight(48)),icon:const Icon(Icons.save_rounded),label:Text(_saving?(widget.english?'Saving...':'جاري الحفظ...'):(widget.english?'Save profile':'حفظ الملف'))),
      const SizedBox(height:18),const Divider(),const SizedBox(height:8),
      OutlinedButton.icon(onPressed:_terminate,style:OutlinedButton.styleFrom(foregroundColor:Colors.redAccent,side:const BorderSide(color:Colors.redAccent),minimumSize:const Size.fromHeight(46)),icon:const Icon(Icons.person_off_rounded),label:Text(widget.english?'End affiliate contract':'إنهاء التعاقد')),
    ]);
  }

  Widget _profileField(TextEditingController controller,String label,{bool required=false,int maxLines=1})=>Padding(padding:const EdgeInsets.only(bottom:9),child:TextField(controller:controller,maxLines:maxLines,textAlign:TextAlign.start,decoration:InputDecoration(labelText:required?'$label *':label,border:OutlineInputBorder(borderRadius:BorderRadius.circular(12)))));
}
